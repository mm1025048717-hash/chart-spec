/**
 * 批量更新所有图表文件的筛选系统
 * 将分散的filter-item和filter-select替换为统一筛选按钮
 */

const fs = require('fs');
const path = require('path');

// 需要更新的图表目录
const chartDirs = [
    '02-stacked-column',
    '03-bar',
    '04-line',
    '05-area',
    '06-pie',
    '07-histogram',
    '08-scatter',
    '09-bubble',
    '10-radar',
    '11-funnel',
    '12-gauge',
    '13-pareto',
    '14-treemap',
    '15-wordcloud',
    '16-heatmap',
    '17-map',
    '19-indicator',
    '20-ring',
    '21-waterfall',
    '22-dual-axes',
    '23-rose',
    '24-sunburst',
    '25-sankey',
    '26-progress'
];

// 统一的筛选按钮HTML模板
const unifiedFilterHTML = `
            <div class="chart-filters">
                <div class="filter-wrapper">
                    <button class="filter-unified-btn" onclick="toggleFilterPanel(event)">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"></polygon>
                        </svg>
                        筛选
                        <span class="filter-count" id="filterCount">0</span>
                    </button>
                    <!-- 统一筛选面板 -->
                    <div class="filter-modal" id="filterModal" onclick="event.stopPropagation()">
                        <div class="filter-panel">
                            <div class="filter-panel-header" id="filterPanelHeader">
                                <div class="filter-panel-title">
                                    <div class="filter-panel-drag-handle">
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                            <circle cx="9" cy="5" r="1"></circle>
                                            <circle cx="9" cy="12" r="1"></circle>
                                            <circle cx="9" cy="19" r="1"></circle>
                                            <circle cx="15" cy="5" r="1"></circle>
                                            <circle cx="15" cy="12" r="1"></circle>
                                            <circle cx="15" cy="19" r="1"></circle>
                                        </svg>
                                    </div>
                                    筛选
                                </div>
                                <button class="filter-panel-close" onclick="closeFilterPanel()">×</button>
                            </div>
                            <div class="filter-panel-body">
                                <div class="filter-dimension collapsed">
                                    <div class="filter-dimension-header" onclick="toggleFilterDimension(this)">
                                        <div class="filter-dimension-title">维度</div>
                                        <span class="filter-dimension-toggle">▶</span>
                                    </div>
                                    <div class="filter-dimension-content">
                                        <div class="filter-options" id="filterDimension">
                                            <div class="filter-option" data-value="选项1" onclick="toggleFilterOption(this, 'dimension', '选项1')">选项1</div>
                                            <div class="filter-option" data-value="选项2" onclick="toggleFilterOption(this, 'dimension', '选项2')">选项2</div>
                                        </div>
                                    </div>
                                </div>
                                <div class="filter-dimension collapsed">
                                    <div class="filter-dimension-header" onclick="toggleFilterDimension(this)">
                                        <div class="filter-dimension-title">日期</div>
                                        <span class="filter-dimension-toggle">▶</span>
                                    </div>
                                    <div class="filter-dimension-content">
                                        <div class="filter-options" id="filterDate">
                                            <div class="filter-option" data-value="2024.12" onclick="toggleFilterOption(this, 'date', '2024.12')">2024.12</div>
                                            <div class="filter-option" data-value="2024.Q4" onclick="toggleFilterOption(this, 'date', '2024.Q4')">2024.Q4</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="filter-panel-footer">
                                <button class="filter-btn filter-btn-reset" onclick="resetFilters()">重置</button>
                                <button class="filter-btn filter-btn-apply" onclick="applyFilters()">应用</button>
                            </div>
                        </div>
                    </div>
                </div>
                <!-- 筛选摘要 -->
                <div class="filter-summary" id="filterSummary"></div>
            </div>`;

function updateChartFile(filePath) {
    try {
        let content = fs.readFileSync(filePath, 'utf8');
        
        // 检查是否已经包含统一筛选系统
        if (content.includes('filter-unified-btn')) {
            console.log(`跳过 ${filePath} - 已包含统一筛选系统`);
            return;
        }
        
        // 检查是否包含script标签引用unified-filter.js
        if (!content.includes('unified-filter.js')) {
            // 在responsive-spec.js之后添加unified-filter.js
            content = content.replace(
                /<script src="\.\.\/responsive-spec\.js"><\/script>/,
                '<script src="../responsive-spec.js"></script>\n    <script src="../unified-filter.js"></script>'
            );
        }
        
        // 替换分散的筛选控件
        const filterItemPattern = /<div class="chart-filters">[\s\S]*?<\/div>\s*<\/div>/;
        if (filterItemPattern.test(content)) {
            content = content.replace(filterItemPattern, unifiedFilterHTML);
            console.log(`已更新 ${filePath}`);
        } else {
            console.log(`未找到筛选控件 ${filePath}`);
        }
        
        // 在脚本末尾添加初始化代码
        if (!content.includes('initUnifiedFilter')) {
            const scriptEndPattern = /(\/\/ 初始化响应式\s*ChartSpec\.init\(\);)/;
            if (scriptEndPattern.test(content)) {
                content = content.replace(
                    scriptEndPattern,
                    `$1\n\n// 初始化统一筛选系统\ndocument.addEventListener('DOMContentLoaded', function() {\n    initUnifiedFilter({\n        dimensions: [\n            { key: 'dimension', type: 'multi' },\n            { key: 'date', type: 'multi' }\n        ],\n        onApply: function(state) {\n            console.log('筛选已应用:', state);\n        }\n    });\n});`
                );
            }
        }
        
        fs.writeFileSync(filePath, content, 'utf8');
    } catch (error) {
        console.error(`更新 ${filePath} 时出错:`, error.message);
    }
}

// 更新所有图表文件
chartDirs.forEach(dir => {
    const filePath = path.join(__dirname, dir, 'index.html');
    if (fs.existsSync(filePath)) {
        updateChartFile(filePath);
    } else {
        console.log(`文件不存在: ${filePath}`);
    }
});

console.log('批量更新完成！');
