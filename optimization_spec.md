...
| 多数据项 (≥15项) | `-Math.PI / 4` | `end` | **优先 45°倾斜 + 截断**（可读性更好） |
| 超长文本或 45°仍重叠 | `-Math.PI / 2` | `end` | 90°垂直 + 截断（仅作兜底，阅读需歪头） |

**说明**：90° 垂直标签虽省空间，但阅读体验差（需歪头），建议多数据项时**优先使用 45° + 截断**；仅当标签过长或项数很多（如 20+）导致 45° 仍明显重叠时，再考虑 90°。
